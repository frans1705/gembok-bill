...
function formatWithHeaderFooter(message) {
    try {
        ...
        const header = getSetting('company_header', 'NANCY.NET BOT MANAGEMENT ISP');
        ...
        return `🏢 *NANCY.NET BOT MANAGEMENT ISP*\n\n${message}\n\nInternet Tanpa Batas`;
    }
}
...
async function connectToWhatsApp() {
    try {
        ...
        sock = makeWASocket({
            auth: state,
            logger,
            browser: ['NANCY GLOBAL SOLUSI', 'Chrome', '1.0.0'],
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    // Pesan notifikasi
                    const notificationMessage = `📱 *BOT WHATSAPP NANCY.NET*\n\n` +
                    ...
                    `💰 *Dukungan Pengembang:*\n` +
                    `• E-WALLET: 081290717151\n` +
                    `• BCA: 4060623347 a.n FRANS WELLEM TUMILAAR\n\n` +
...
▸ *setheader [teks_header_baru]* — Ganti header pesan bot
   Contoh: setheader NANCY.NET HOTSPOT
▸ *setfooter [teks_footer_baru]* — Ganti footer pesan bot
   Contoh: setfooter Powered by NANCY GLOBAL SOLUSI
...
async function sendWelcomeMessage(remoteJid, isAdmin = false) {
    try {
        ...
        let welcomeMessage = `👋 *Selamat Datang di Bot WhatsApp ${getSetting('company_header', 'NANCY.NET BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n\n`;
        ...
        // Tambahkan footer
        welcomeMessage += `🏢 *${getSetting('company_header', 'NANCY.NET BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n`;
...
async function handleAdminMenu(remoteJid) {
    try {
        ...
        // Tambahkan footer
        adminMessage += `🏢 *${getSetting('company_header', 'NANCY.NET BOT MANAGEMENT ISP') || 'ISP Monitor'}*\n`;
